...
For large builds, open **Preview settings** from the top-right menu to adjust chunk separation and the decoder memory limit. The limit is off by default; enabling it starts at 2048 MB and accepts integer values through 8192 MB. It remains effective with either multithreaded scheduling mode. It applies only to the isolated decoder process, not the host, WebView2 or GPU. If decoding stops while a cap is enabled, loading returns to Home with an error dialog; a decoder crash alone does not prove that the cap was reached. Changes apply on the next file open.
...
**Speed first** is an additional multithreading option, off by default. It bypasses memory-first mesh concurrency reduction, refills consumed native task slots immediately, and allows the selected number of queued host batches and upload preparation pages (up to 8 rather than the memory-first upload limit of 4). The optional decoder process-memory cap remains effective when enabled; reaching it may stop the decoder and fail the load. Work remains ordered and bounded by worker count, and each IPC page is still at most 1 MiB; this is not an unlimited producer queue. Large schematics can nevertheless exhaust system memory or crash the decoder, and more concurrency does not guarantee a faster load.
